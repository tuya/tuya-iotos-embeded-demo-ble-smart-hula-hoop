 ...
 * @return LED_RET
 */
LED_RET tuya_create_led_handle(IN CONST TY_GPIO_PORT_E pin, IN CONST UCHAR_T active_low, OUT LED_HANDLE *handle)
{
    /* check handle */
    if (NULL == handle) {
...
LED_RET tuya_led_reset(VOID_T)
{
    ...
    tuya_software_timer_delete(__led_timeout_handler);
    tuya_software_timer_create(LED_TIMER_VAL_MS*1000, __led_timeout_handler);
    return LED_OK;
}
